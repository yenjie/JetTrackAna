...
//     TFile *inf = new TFile("../mptTree-80-R5.root");
     TFile *inf = new TFile("../mptTreeFFCor-80-R4.root");
...
//   t->SetAlias("LD","leadingJetPt*0.00760+leadingJetNTrk4*(-0.00304)+subleadingJetPt*(-0.00883)+subleadingJetNTrk4*0.01045-0.0272");
   for (int i=0;i<nCentBins;i++) {
      ...
      TProfile *pSubleading = new TProfile(Form("pSubleading%d",i),"",50,0,1);
      TCanvas *c2 = new TCanvas(Form("c2_%i",i),Form("Subleading Jet Matching Probability %d-%d",centBins[i],centBins[i+1]),600,600);
      pSubleading->SetXTitle("Reco A_{J, 23}");
      pSubleading->SetYTitle("Sublading Jet Matching Prob");
      t->Draw(Form("(subleadingJetRefPt==gensubleadingJetPt):(subleadingJetPt-thirdleadingJetPt)/(subleadingJetPt+thirdleadingJetPt)>>pSubleading%d",i),centCut&&"leadingJetRefPt==genleadingJetPt&&leadingJetPt>120&&subleadingJetPt>50&&thirdleadingJetPt>50&&acos(cos(leadingJetPhi-subleadingJetPhi))>2*3.14159/3.","prof");
      // matching probability between leading reco jet and leading gen jet
      TF1 *fPSubleading = new TF1(Form("fPSubleading_%d_%d",centBins[i],centBins[i+1]),"TMath::Erf(x*[0])*0.5+0.5");
      pSubleading->Fit(Form("fPSubleading_%d_%d",centBins[i],centBins[i+1]));
      pSubleading->Fit(Form("fPSubleading_%d_%d",centBins[i],centBins[i+1]));
      pSubleading->Fit(Form("fPSubleading_%d_%d",centBins[i],centBins[i+1]));
      pSubleading->Fit(Form("fPSubleading_%d_%d",centBins[i],centBins[i+1]));
      fPSubleading->Write();
