...
void plotAnaMultSys(char *infname="ana.root")
{    
...
//   c->Divide(4,1);
   ...
   TCut recoCut2 = "leadingJetPt>120*1.05&&subleadingJetPt>50*1.05&&dphi>5*3.14159265358979/6.&&abs(leadingJetEta)<1.6&&abs(subleadingJetEta)<1.6";
//   TCut recoCut3 = "leadingJetPt>120*0.95&&subleadingJetPt>50*0.95&&dphi>5*3.14159265358979/6.&&abs(leadingJetEta)<1.6&&abs(subleadingJetEta)<1.6";
   ...
   for (int i=0;i<4;i++) {
      ...
      TProfile *pData2 = new TProfile(Form("pData2%d",i),"",nAjBin,AjBins);
      TProfile *pData3 = new TProfile(Form("pData3%d",i),"",nAjBin,AjBins);
      ...
      tData->Draw(Form("-multDiff:Aj>>pData2%d",i),recoCut2&&centCut);
//      tData->Draw(Form("-multDiff:Aj>>pData3%d",i),recoCut3&&centCut);
...
/*      
      ...
      */
      ...
      pData2->Draw("same");
      pData3->Draw("same");
      pData2->SetMarkerColor(4);
      pData2->SetMarkerStyle(24);
...
//      TProfile *pDiff = (TProfile*)pData->Clone("pDiff");
      ...
      for (int j=1;j<=pData->GetNbinsX();j++)
      {
	 pDiff->SetBinContent(j,pData2->GetBinContent(j)-pData->GetBinContent(j));
  	 pDiff->SetBinError(j,sqrt(pData->GetBinError(j)*pData->GetBinError(j)));
...
//	 b->Draw();
	 b2->Draw();
      ...
      leg->SetFillStyle(0);
      leg->SetBorderSize(0);
      leg->AddEntry(pData,"Data","pl");
      leg->AddEntry(pData2,"Data JEC +5%","pl");
      if (i==0) leg->Draw();
...
*/
   ...
   c1->SaveAs("results/MultiplicityDifference-AJ-SysJEC.C");
   c1->SaveAs("results/MultiplicityDifference-AJ-SysJEC.gif");
   c1->SaveAs("results/MultiplicityDifference-AJ-SysJEC.eps");
   c1->SaveAs("results/MultiplicityDifference-AJ-SysJEC.pdf");
