...
void plotAnaMult5(char *infname="ana.root")
{    
...
//   c->Divide(4,1);
   ...
   const int nPtBin=4;
   Float_t PtBins[nPtBin+1] = {120,150,200,250,350};
   ...
   for (int i=0;i<4;i++) {
      c1->cd(i+1);
      TH1D * empty=new TH1D("empty","",100,120,350);
//      empty->Fill(0.5,1000); 
      ...
      tData->Draw(Form("-multDiff:leadingJetPt>>pData%d",i),recoCut&&centCut);
      tPP->Draw(Form("-multDiff:leadingJetPt>>pPP%d",i),recoCut);
      tMC->Draw(Form("-multDiff:leadingJetPt>>pMC%d",i),recoCut&&centCut);  
      tMC->Draw(Form("-genMultDiff:genleadingJetPt>>pGen%d",i),genCut&&centCut);  
...
*/
   ...
   c1->SaveAs("results/MultiplicityDifference-pT1.C");
   c1->SaveAs("results/MultiplicityDifference-pT1.gif");
   c1->SaveAs("results/MultiplicityDifference-pT1.eps");
   c1->SaveAs("results/MultiplicityDifference-pT1.pdf");
